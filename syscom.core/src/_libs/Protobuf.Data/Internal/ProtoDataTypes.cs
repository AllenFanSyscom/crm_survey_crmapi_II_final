...
namespace libs.ProtoBuf.Data.Internal
{
	...
	internal static class ProtoDataTypes
	{
		static ProtoDataTypes()
		{
			...
			for ( var i = 0; i < values.Length; i++ ) AllTypes[i] = (ProtoDataType) values.GetValue( i );

			AllClrTypes = new Type[values.Length];
			for ( var i = 0; i < AllTypes.Length; i++ ) AllClrTypes[i] = ConvertProtoDataType.ToClrType( AllTypes[i] );
		}
