...
namespace libs.ProtoBuf.Data
{
	...
	public class ProtoDataWriter : IProtoDataWriter
	{
		...
		public void Serialize( Stream stream, DataSet dataSet ) { Serialize( stream, dataSet.CreateDataReader(), new ProtoDataWriterOptions() ); }
		...
		/// <param name="stream">The <see cref="System.IO.Stream"/> to write to.</param>
		/// <param name="dataSet">The <see cref="System.Data.DataSet"/>who's contents to serialize.</param>
		public void Serialize( Stream stream, DataSet dataSet, ProtoDataWriterOptions options ) { Serialize( stream, dataSet.CreateDataReader(), options ); }
		...
		public void Serialize( Stream stream, DataTable dataTable ) { Serialize( stream, dataTable.CreateDataReader(), new ProtoDataWriterOptions() ); }
		...
		/// <param name="stream">The <see cref="System.IO.Stream"/> to write to.</param>
		/// <param name="dataTable">The <see cref="System.Data.DataTable"/>who's contents to serialize.</param>
		public void Serialize( Stream stream, DataTable dataTable, ProtoDataWriterOptions options ) { Serialize( stream, dataTable.CreateDataReader(), options ); }
		...
		public void Serialize( Stream stream, IDataReader reader, ProtoDataWriterOptions options )
		{
			...
			using ( var writer = new ProtoWriter( stream, null, null ) )
			{
				do
				{
					...
					// write the rows
					while ( reader.Read() ) rowWriter.WriteRow( reader );
