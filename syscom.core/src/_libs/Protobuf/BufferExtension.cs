...
namespace libs.ProtoBuf
{
	...
	public sealed class BufferExtension : IExtension
	{
		...
		Int32 IExtension.GetLength() { return buffer == null ? 0 : buffer.Length; }

		Stream IExtension.BeginAppend() { return new MemoryStream(); }

		void IExtension.EndAppend( Stream stream, Boolean commit )
		{
			using ( stream )
			{
				Int32 len;
				if ( commit && ( len = (Int32) stream.Length ) > 0 )
				{
		...
		Stream IExtension.BeginQuery() { return buffer == null ? Stream.Null : new MemoryStream( buffer ); }
