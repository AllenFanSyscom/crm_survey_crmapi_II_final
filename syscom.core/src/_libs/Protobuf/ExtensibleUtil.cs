...
namespace libs.ProtoBuf
{
		...
		class ExtensibleUtil
	{
		...
		/// </summary>
		internal static IEnumerable<TValue> GetExtendedValues<TValue>( IExtensible instance, Int32 tag, DataFormat format, Boolean singleton, Boolean allowDefinedTag )
		{
			foreach ( TValue value in GetExtendedValues( RuntimeTypeModel.Default, typeof( TValue ), instance, tag, format, singleton, allowDefinedTag ) ) yield return value;
		}
#endif
		/// <summary>
		/// All this does is call GetExtendedValuesTyped with the correct type for "instance";
		...
		/// the caller must respect the fields defined for the type they pass in.
		/// </summary>
		internal static IEnumerable GetExtendedValues( TypeModel model, Type type, IExtensible instance, Int32 tag, DataFormat format, Boolean singleton, Boolean allowDefinedTag )
		{
#if FEAT_IKVM
		...
		internal static void AppendExtendValue( TypeModel model, IExtensible instance, Int32 tag, DataFormat format, Object value )
		{
#if FEAT_IKVM
            throw new NotSupportedException();
#else
