...
namespace libs.ProtoBuf.Meta
{
	...
	public sealed class RuntimeTypeModel : TypeModel
	{
		...
		Boolean GetOption( Byte option ) { return ( options & option ) == option; }

		void SetOption( Byte option, Boolean value )
		{
		...
		/// impacted type.
		/// </summary>
		public Boolean InferTagFromNameDefault { get => GetOption( OPTIONS_InferTagFromNameDefault ); set => SetOption( OPTIONS_InferTagFromNameDefault, value ); }
		...
		/// </summary>
		public Boolean AutoAddProtoContractTypesOnly { get => GetOption( OPTIONS_AutoAddProtoContractTypesOnly ); set => SetOption( OPTIONS_AutoAddProtoContractTypesOnly, value ); }
		...
		/// </summary>
		public Boolean UseImplicitZeroDefaults
		{
			get => GetOption( OPTIONS_UseImplicitZeroDefaults );
			set
			{
				if ( !value && GetOption( OPTIONS_IsDefaultModel ) ) throw new InvalidOperationException( "UseImplicitZeroDefaults cannot be disabled on the default model" );
		...
		/// </summary>
		public Boolean AllowParseableTypes { get => GetOption( OPTIONS_AllowParseableTypes ); set => SetOption( OPTIONS_AllowParseableTypes, value ); }
		...
		public override String GetSchema( Type type )
		{
			...
			else
			{
				...
				if ( !isInbuiltType )
				{
					...
					var index = FindOrAddAuto( type, false, false, false );
			...
			var requiresBclImport = false;
			var bodyBuilder = new StringBuilder();
			// sort them by schema-name
			var metaTypesArr = new MetaType[requiredTypes.Count];
			...
			{
				for ( var i = 0; i < metaTypesArr.Length; i++ )
				{
					var tmp = metaTypesArr[i];
		...
		void CascadeDependents( BasicList list, MetaType metaType )
		{
			...
			if ( metaType.IsList )
			{
				...
				{
					var index = FindOrAddAuto( itemType, false, false, false );
					if ( index >= 0 )
					{
				...
				if ( metaType.IsAutoTuple )
				{
					...
					if ( MetaType.ResolveTupleConstructor( metaType.Type, out mapping ) != null )
						for ( var i = 0; i < mapping.Length; i++ )
						{
							...
							WireType defaultWireType;
							var coreSerializer = ValueMember.TryGetCoreSerializer( this, DataFormat.Default, type, out defaultWireType, false, false, false, false );
							if ( coreSerializer == null )
							{
								var index = FindOrAddAuto( type, false, false, false );
								if ( index >= 0 )
		...
		internal RuntimeTypeModel( Boolean isDefault )
		{
#if FEAT_IKVM
		...
		internal MetaType FindWithoutAdd( Type type )
		{
			...
			var underlyingType = ResolveProxies( type );
		...
		static Boolean MetaTypeFinderImpl( Object value, Object ctx ) { return ( (MetaType) value ).Type == (Type) ctx; }
		static Boolean BasicTypeFinderImpl( Object value, Object ctx ) { return ( (BasicType) value ).Type == (Type) ctx; }
		...
		sealed class BasicType
		{
			...
			readonly IProtoSerializer serializer;
		...
		{
			var idx = basicTypes.IndexOf( BasicTypeFinder, type );
			...
			lock ( basicTypes )
			{
				...
				var ser = family == MetaType.AttributeFamily.None
		...
		internal Int32 FindOrAddAuto( Type type, Boolean demand, Boolean addWithContractOnly, Boolean addEvenIfAutoDisabled )
		{
			var key = types.IndexOf( MetaTypeFinder, type );
			MetaType metaType;
			...
			var shouldAdd = AutoAddMissingTypes || addEvenIfAutoDisabled;
			...
			if ( key < 0 )
			{
				...
				try
				{
					...
					if ( ( metaType = RecogniseCommonTypes( type ) ) == null )
					{
						...
						var family = MetaType.GetContractFamily( this, type, null );
						if ( family == MetaType.AttributeFamily.AutoTuple ) shouldAdd = addEvenIfAutoDisabled = true; // always add basic tuples, such as KeyValuePair
		...
		public MetaType Add( Type type, Boolean applyDefaultBehaviour )
		{
			if ( type == null ) throw new ArgumentNullException( "type" );
			var newType = FindWithoutAdd( type );
		...
		/// are encountered.
		/// </summary>
		public Boolean AutoAddMissingTypes
		{
			get => GetOption( OPTIONS_AutoAddMissingTypes );
			set
			{
				if ( !value && GetOption( OPTIONS_IsDefaultModel ) ) throw new InvalidOperationException( "The default model must allow missing types" );
				ThrowIfFrozen();
		...
		readonly BasicList types = new BasicList();

		/// <summary>
		/// Provides the key that represents a given type in the current model.
		/// </summary>
		protected override Int32 GetKeyImpl( Type type ) { return GetKey( type, false, true ); }

		internal Int32 GetKey( Type type, Boolean demand, Boolean getBaseKey )
		{
			Helpers.DebugAssert( type != null );
			try
			{
				var typeIndex = FindOrAddAuto( type, demand, true, false );
				if ( typeIndex >= 0 )
				{
		...
		protected internal override void Serialize( Int32 key, Object value, ProtoWriter dest )
		{
#if FEAT_IKVM
		...
		protected internal override Object Deserialize( Int32 key, Object value, ProtoReader source )
		{
#if FEAT_IKVM
            throw new NotSupportedException();
#else
...
,
		...
		internal Boolean IsPrepared( Type type )
		{
			var meta = FindWithoutAdd( type );
		...
		Int32 metadataTimeoutMilliseconds = 5000;

		/// <summary>
		/// The amount of time to wait if there are concurrent metadata access operations
		/// </summary>
		public Int32 MetadataTimeoutMilliseconds
		{
		...
		/// </summary>
		public Int32 LockCount => lockCount;
#endif
		internal void TakeLock( ref Int32 opaqueToken )
		{
		...
		internal void ReleaseLock( Int32 opaqueToken )
		{
			if ( opaqueToken != 0 )
			{
				Monitor.Exit( types );
		...
		internal void ResolveListTypes( Type type, ref Type itemType, ref Type defaultType )
		{
			...
			// handle lists
			if ( itemType == null ) itemType = GetListItemType( this, type );
			...
			if ( itemType != null )
			{
				...
				if ( nestedItemType != null ) throw CreateNestedListsNotSupported();
			}
		...
		internal String GetSchemaTypeName( Type effectiveType, DataFormat dataFormat, Boolean asReference, Boolean dynamicType, ref Boolean requiresBclImport )
		{
			var tmp = Helpers.GetUnderlyingType( effectiveType );
			if ( tmp != null ) effectiveType = tmp;

			if ( effectiveType == MapType( typeof( Byte[] ) ) ) return "bytes";

			WireType wireType;
			var ser = ValueMember.TryGetCoreSerializer( this, dataFormat, effectiveType, out wireType, false, false, false, false );
			if ( ser == null )
			{
				// model type
	...
	{
		readonly String ownerStackTrace;
		internal LockContentedEventArgs( String ownerStackTrace ) { this.ownerStackTrace = ownerStackTrace; }
