...
namespace libs.ProtoBuf.Meta
{
	...
	public abstract class TypeModel
	{
		...
		/// </summary>
		protected internal Type MapType( Type type ) { return MapType( type, true ); }

		/// <summary>
		/// Resolve a System.Type to the compiler-specific type
		/// </summary>
		protected internal virtual Type MapType( Type type, Boolean demand )
		{
#if FEAT_IKVM
		...
		WireType GetWireType( ProtoTypeCode code, DataFormat format, ref Type type, out Int32 modelKey )
		{
			modelKey = -1;
			...
			if ( ( modelKey = GetKey( ref type ) ) >= 0 ) return WireType.String;
		...
		///
		/// </summary>
		internal Boolean TrySerializeAuxiliaryType( ProtoWriter writer, Type type, DataFormat format, Int32 tag, Object value, Boolean isInsideList )
		{
			...
			var wireType = GetWireType( typecode, format, ref type, out modelKey );
			...
			if ( wireType != WireType.None ) ProtoWriter.WriteFieldHeader( tag, wireType, writer );
			switch ( typecode )
			{
				case ProtoTypeCode.Int16:
				...
				case ProtoTypeCode.Int64:
					...
					return true;
				case ProtoTypeCode.UInt16:
					ProtoWriter.WriteUInt16( (UInt16) value, writer );
					return true;
				case ProtoTypeCode.UInt32:
					ProtoWriter.WriteUInt32( (UInt32) value, writer );
					return true;
				case ProtoTypeCode.UInt64:
					ProtoWriter.WriteUInt64( (UInt64) value, writer );
					return true;
				case ProtoTypeCode.Boolean:
				...
				case ProtoTypeCode.Byte:
					...
					return true;
				case ProtoTypeCode.Char:
					ProtoWriter.WriteUInt16( (UInt16) (Char) value, writer );
					return true;
				case ProtoTypeCode.Double:
			...
			if ( sequence != null )
			{
				...
				foreach ( var item in sequence )
				{
					...
					if ( !TrySerializeAuxiliaryType( writer, null, format, tag, item, true ) ) ThrowUnexpectedType( item.GetType() );
				}
		...
		void SerializeCore( ProtoWriter writer, Object value )
		{
			...
			var key = GetKey( ref type );
			if ( key >= 0 )
				Serialize( key, value, writer );
			else if ( !TrySerializeAuxiliaryType( writer, type, DataFormat.Default, Serializer.ListItemTag, value, false ) ) ThrowUnexpectedType( type );
		}
#endif
		/// <summary>
		...
		public void Serialize( Stream dest, Object value ) { Serialize( dest, value, null ); }
...
#else
			using ( var writer = new ProtoWriter( dest, this, context ) )
			{
		...
		public Object DeserializeWithLengthPrefix( Stream source, Object value, Type type, PrefixStyle style, Int32 fieldNumber )
		{
			Int32 bytesRead;
		...
		/// either the original instance was null, or the stream defines a known sub-type of the
		/// original instance.</returns>
		public Object DeserializeWithLengthPrefix( Stream source, Object value, Type type, PrefixStyle style, Int32 expectedField, Serializer.TypeResolver resolver )
		{
			Int32 bytesRead;
		...
		/// either the original instance was null, or the stream defines a known sub-type of the
		/// original instance.</returns>
		public Object DeserializeWithLengthPrefix( Stream source, Object value, Type type, PrefixStyle style, Int32 expectedField, Serializer.TypeResolver resolver, out Int32 bytesRead )
		{
			Boolean haveObject;
			return DeserializeWithLengthPrefix( source, value, type, style, expectedField, resolver, out bytesRead, out haveObject, null );
		}

		Object DeserializeWithLengthPrefix( Stream source, Object value, Type type, PrefixStyle style, Int32 expectedField, Serializer.TypeResolver resolver, out Int32 bytesRead, out Boolean haveObject, SerializationContext context )
		{
#if FEAT_IKVM
            throw new NotSupportedException();
#else
			...
			Int32 tmpBytesRead;
			bytesRead = 0;
			if ( type == null && ( style != PrefixStyle.Base128 || resolver == null ) ) throw new InvalidOperationException( "A type must be provided unless base-128 prefixing is being used in combination with a resolver" );
			Int32 actualField;
			do
			{
			...
			try
			{
				...
				{
					if ( !( TryDeserializeAuxiliaryType( reader, DataFormat.Default, Serializer.ListItemTag, type, ref value, true, false, true, false ) || len == 0 ) ) ThrowUnexpectedType( type ); // throws
				}
		...
		public IEnumerable DeserializeItems( Stream source, Type type, PrefixStyle style, Int32 expectedField, Serializer.TypeResolver resolver ) { return DeserializeItems( source, type, style, expectedField, resolver, null ); }
		...
		/// <returns>The sequence of deserialized objects.</returns>
		/// <param name="context">Additional information about this serialization operation.</param>
		public IEnumerable DeserializeItems( Stream source, Type type, PrefixStyle style, Int32 expectedField, Serializer.TypeResolver resolver, SerializationContext context ) { return new DeserializeItemsIterator( this, source, type, style, expectedField, resolver, context ); }
		...
		/// expected and all records are returned).</param>
		/// <returns>The sequence of deserialized objects.</returns>
		public System.Collections.Generic.IEnumerable<T> DeserializeItems<T>( Stream source, PrefixStyle style, Int32 expectedField ) { return DeserializeItems<T>( source, style, expectedField, null ); }
		...
		/// <returns>The sequence of deserialized objects.</returns>
		/// <param name="context">Additional information about this serialization operation.</param>
		public System.Collections.Generic.IEnumerable<T> DeserializeItems<T>( Stream source, PrefixStyle style, Int32 expectedField, SerializationContext context ) { return new DeserializeItemsIterator<T>( this, source, style, expectedField, context ); }

		sealed class DeserializeItemsIterator<T> : DeserializeItemsIterator,
		                                           ...
		                                           System.Collections.Generic.IEnumerable<T>
		{
			...
			public DeserializeItemsIterator( TypeModel model, Stream source, PrefixStyle style, Int32 expectedField, SerializationContext context )
				: base( model, source, model.MapType( typeof( T ) ), style, expectedField, null, context )
			{
			}
		}
#endif
		class DeserializeItemsIterator : IEnumerator, IEnumerable
		{
			...
			readonly Type type;
			readonly PrefixStyle style;
			readonly Int32 expectedField;
			readonly Serializer.TypeResolver resolver;
			readonly TypeModel model;
			readonly SerializationContext context;

			public DeserializeItemsIterator( TypeModel model, Stream source, Type type, PrefixStyle style, Int32 expectedField, Serializer.TypeResolver resolver, SerializationContext context )
			{
				haveObject = true;
				this.source = source;
				this.type = type;
		...
		/// <param name="dest">The destination stream to write to.</param>
		/// <param name="fieldNumber">The tag used as a prefix to each record (only used with base-128 style prefixes).</param>
		public void SerializeWithLengthPrefix( Stream dest, Object value, Type type, PrefixStyle style, Int32 fieldNumber ) { SerializeWithLengthPrefix( dest, value, type, style, fieldNumber, null ); }
		...
		/// <param name="fieldNumber">The tag used as a prefix to each record (only used with base-128 style prefixes).</param>
		/// <param name="context">Additional information about this serialization operation.</param>
		public void SerializeWithLengthPrefix( Stream dest, Object value, Type type, PrefixStyle style, Int32 fieldNumber, SerializationContext context )
		{
			if ( type == null )
			{
				if ( value == null ) throw new ArgumentNullException( "value" );
		...
		/// either the original instance was null, or the stream defines a known sub-type of the
		/// original instance.</returns>
		public Object Deserialize( Stream source, Object value, Type type ) { return Deserialize( source, value, type, null ); }
		...
		Boolean PrepareDeserialize( Object value, ref Type type )
		{
			if ( type == null )
			{
				if ( value == null )
			...
			var autoCreate = true;
#if !NO_GENERICS
			var underlyingType = Helpers.GetUnderlyingType( type );
		...
		/// either the original instance was null, or the stream defines a known sub-type of the
		/// original instance.</returns>
		public Object Deserialize( Stream source, Object value, Type type, Int32 length ) { return Deserialize( source, value, type, length, null ); }
		...
		/// original instance.</returns>
		/// <param name="context">Additional information about this serialization operation.</param>
		public Object Deserialize( Stream source, Object value, Type type, Int32 length, SerializationContext context )
		{
#if FEAT_IKVM
            throw new NotSupportedException();
#else
		...
		public Object Deserialize( ProtoReader source, Object value, Type type )
		{
			...
			var obj = DeserializeCore( source, type, value, autoCreate );
...
#if !FEAT_IKVM
		Object DeserializeCore( ProtoReader reader, Type type, Object value, Boolean noAutoCreate )
		{
			var key = GetKey( ref type );
			if ( key >= 0 && !Helpers.IsEnum( type ) ) return Deserialize( key, value, reader );
...
#endif
		internal static MethodInfo ResolveListAdd( TypeModel model, Type listType, Type itemType, out Boolean isList )
		{
#if WINRT
			...
			if ( add == null )
			{
				...
				var forceList = listTypeInfo.IsInterface &&
...
#else
				var constuctedListType = model.MapType( typeof( System.Collections.Generic.ICollection<> ) ).MakeGenericType( types );
#endif
				if ( forceList || constuctedListType.IsAssignableFrom( listTypeInfo ) ) add = Helpers.GetInstanceMethod( constuctedListType, "Add", types );
			}
		...
		internal static Type GetListItemType( TypeModel model, Type listType )
		{
...
#endif
			{
				...
				Type paramType;
				if ( parameters.Length == 1 && !candidates.Contains( paramType = parameters[0].ParameterType ) ) candidates.Add( paramType );
			}
...
#else
				foreach ( var iType in listType.GetInterfaces() ) TestEnumerableListPatterns( model, candidates, iType );
#endif
			}
#endif
...
#else
			// more convenient GetProperty overload not supported on all platforms
			foreach ( var indexer in listType.GetProperties( BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic ) )
			{
				if ( indexer.Name != "Item" || candidates.Contains( indexer.PropertyType ) ) continue;
				var args = indexer.GetIndexParameters();
				if ( args.Length != 1 || args[0].ParameterType != model.MapType( typeof( Int32 ) ) ) continue;
				candidates.Add( indexer.PropertyType );
			}
#endif
		...
		static void TestEnumerableListPatterns( TypeModel model, BasicList candidates, Type iType )
		{
#if WINRT
			...
			if ( iType.IsGenericType )
			{
				     ...
				     || typeDef.FullName == "System.Collections.Concurrent.IProducerConsumerCollection`1" )
				{
					...
					if ( !candidates.Contains( iTypeArgs[0] ) ) candidates.Add( iTypeArgs[0] );
				}
			}
#endif
		}

		static Boolean CheckDictionaryAccessors( TypeModel model, Type pair, Type value )
		{
#if NO_GENERICS
            return false;
#elif WINRT
...
#if !FEAT_IKVM
		Boolean TryDeserializeList( TypeModel model, ProtoReader reader, DataFormat format, Int32 tag, Type listType, Type itemType, ref Object value )
		{
			Boolean isList;
			var addMethod = ResolveListAdd( model, listType, itemType, out isList );
			if ( addMethod == null ) throw new NotSupportedException( "Unknown list variant: " + listType.FullName );
			var found = false;
			Object? nextItem = null;
			var list = value as IList;
			var args = isList ? null : new Object[1];
			var arraySurrogate = listType.IsArray ? new BasicList() : null;
		...
		static Object CreateListInstance( Type listType, Type itemType )
		{
		...
		/// </summary>
		internal Boolean TryDeserializeAuxiliaryType( ProtoReader reader, DataFormat format, Int32 tag, Type type, ref Object value, Boolean skipOtherFields, Boolean asListItem, Boolean autoCreate, Boolean insideList )
		{
			...
			if ( wiretype == WireType.None )
			{
				...
				if ( itemType == null && type.IsArray && type.GetArrayRank() == 1 && type != typeof( Byte[] ) ) itemType = type.GetElementType();
				if ( itemType != null )
				{
					if ( insideList ) throw CreateNestedListsNotSupported();
					found = TryDeserializeList( this, reader, format, tag, type, itemType, ref value );
					if ( !found && autoCreate ) value = CreateListInstance( type, itemType );
					return found;
				}
		...
		/// optimal performance.
		/// </summary>
		public static RuntimeTypeModel Create() { return new RuntimeTypeModel( false ); }
#endif
		...
		/// </summary>
		public Boolean IsDefined( Type type ) { return GetKey( ref type ) >= 0; }

		/// <summary>
		/// Provides the key that represents a given type in the current model.
		/// The type is also normalized for proxies at the same time.
		/// </summary>
		protected internal Int32 GetKey( ref Type type )
		{
			if ( type == null ) return -1;
			var key = GetKeyImpl( type );
			if ( key < 0 )
			{
				var normalized = ResolveProxies( type );
				if ( normalized != null )
				{
		...
		/// Provides the key that represents a given type in the current model.
		/// </summary>
		protected abstract Int32 GetKeyImpl( Type type );
		...
		/// <param name="value">The existing instance to be serialized (cannot be null).</param>
		/// <param name="dest">The destination stream to write to.</param>
		protected internal abstract void Serialize( Int32 key, Object value, ProtoWriter dest );
		...
		/// either the original instance was null, or the stream defines a known sub-type of the
		/// original instance.</returns>
		protected internal abstract Object Deserialize( Int32 key, Object value, ProtoReader source );
		...
		public Object DeepClone( Object value )
		{
			...
			if ( key >= 0 && !Helpers.IsEnum( type ) )
				...
				{
					using ( var writer = new ProtoWriter( ms, this, null ) )
					{
						writer.SetRootObject( value );
						Serialize( key, value, writer );
						writer.Close();
		...
		{
			if ( expected != ResolveProxies( actual ) ) throw new InvalidOperationException( "Unexpected sub-type: " + actual.FullName );
		}
		...
		{
			var fullName = type == null ? "(unknown)" : type.FullName;
		...
		internal static Exception CreateNestedListsNotSupported() { return new NotSupportedException( "Nested or jagged lists and arrays are not supported" ); }

		/// <summary>
		/// Indicates that the given type cannot be constructed; it may still be possible to
		/// deserialize into existing instances.
		/// </summary>
		public static void ThrowCannotCreateInstance( Type type ) { throw new ProtoException( "No parameterless constructor found for " + ( type == null ? "(null)" : type.Name ) ); }
		...
		internal static String SerializeType( TypeModel model, Type type )
		{
			...
			if ( handler != null )
			{
				var args = new TypeFormatEventArgs( type );
				handler( model, args );
		...
		internal static Type DeserializeType( TypeModel model, String value )
		{
			...
			if ( handler != null )
			{
				var args = new TypeFormatEventArgs( value );
				handler( model, args );
		...
		public Boolean CanSerializeContractType( Type type ) { return CanSerialize( type, false, true, true ); }
		...
		/// </summary>
		public Boolean CanSerialize( Type type ) { return CanSerialize( type, true, true, true ); }

		/// <summary>
		/// Returns true if the type supplied is a basic type with inbuilt handling,
		/// or a *list* of a basic type with inbuilt handling
		/// </summary>
		public Boolean CanSerializeBasicType( Type type ) { return CanSerialize( type, true, false, true ); }

		Boolean CanSerialize( Type type, Boolean allowBasic, Boolean allowContract, Boolean allowLists )
		{
			if ( type == null ) throw new ArgumentNullException( "type" );
			var tmp = Helpers.GetUnderlyingType( type );
		...
		Boolean forwardsOnly;
		/// <summary>
		/// If true, buffering of nested objects is disabled
		/// </summary>
		public Boolean ForwardsOnly { get => forwardsOnly; set => forwardsOnly = value; }
#endif
		...
		internal static Type ResolveKnownType( String name, TypeModel model, Assembly assembly )
		{
			...
			{
				var i = name.IndexOf( ',' );
				var fullName = ( i > 0 ? name.Substring( 0, i ) : name ).Trim();
#if !(WINRT || FEAT_IKVM)
				if ( assembly == null ) assembly = Assembly.GetCallingAssembly();
#endif
				var type = assembly == null ? null : assembly.GetType( fullName );
				if ( type != null ) return type;
			}
			catch { }
