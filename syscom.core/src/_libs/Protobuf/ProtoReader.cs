...
namespace libs.ProtoBuf
{
	...
	public sealed class ProtoReader : IDisposable
	{
		...
		Byte[] ioBuffer;
		TypeModel model;
		Int32 fieldNumber, depth, dataRemaining, ioIndex, position, available, blockEnd;
		WireType wireType;
		Boolean isFixedLength, internStrings;
		...
		UInt32 trapCount; // uint is so we can use beq/bne more efficiently than bgt
		...
		public ProtoReader( Stream source, TypeModel model, SerializationContext context ) { Init( this, source, model, context, TO_EOF ); }

		internal const Int32 TO_EOF = -1;
		...
		/// </summary>
		public Boolean InternStrings { get => internStrings; set => internStrings = value; }
		...
		public ProtoReader( Stream source, TypeModel model, SerializationContext context, Int32 length ) { Init( this, source, model, context, length ); }

		static void Init( ProtoReader reader, Stream source, TypeModel model, SerializationContext context, Int32 length )
		{
			...
			reader.blockEnd = Int32.MaxValue;
		...
		internal Int32 TryReadUInt32VariantWithoutMoving( Boolean trimNegative, out UInt32 value )
		{
		...
		UInt32 ReadUInt32Variant( Boolean trimNegative )
		{
			UInt32 value;
		...
		Boolean TryReadUInt32Variant( out UInt32 value )
		{
		...
		public UInt32 ReadUInt32()
		{
			switch ( wireType )
			{
				...
				case WireType.Fixed32:
					...
					position += 4;
					available -= 4;
					return (UInt32) ioBuffer[ioIndex++]
					       | ( (UInt32) ioBuffer[ioIndex++] << 8 )
					       | ( (UInt32) ioBuffer[ioIndex++] << 16 )
					       | ( (UInt32) ioBuffer[ioIndex++] << 24 );
				case WireType.Fixed64:
					var val = ReadUInt64();
					checked { return (UInt32) val; }
				default:
		...
		internal void Ensure( Int32 count, Boolean strict )
		{
			...
			count -= available;
			Int32 writePos = ioIndex + available, bytesRead;
			var canRead = ioBuffer.Length - writePos;
			if ( isFixedLength )
			...
			if ( strict && count > 0 ) throw EoF( this );
		}
		...
		{
			checked { return (Int16) ReadInt32(); }
		}
		...
		{
			checked { return (UInt16) ReadUInt32(); }
		}
		...
		public Int32 ReadInt32()
		{
			...
			{
				case WireType.Variant:
					return (Int32) ReadUInt32Variant( true );
				case WireType.Fixed32:
					if ( available < 4 ) Ensure( 4, true );
					position += 4;
					available -= 4;
					return (Int32) ioBuffer[ioIndex++]
					       | ( (Int32) ioBuffer[ioIndex++] << 8 )
					       | ( (Int32) ioBuffer[ioIndex++] << 16 )
					       | ( (Int32) ioBuffer[ioIndex++] << 24 );
				case WireType.Fixed64:
					var l = ReadInt64();
					checked { return (Int32) l; }
		...
		const Int64 Int64Msb = (Int64) 1 << 63;
		const Int32 Int32Msb = (Int32) 1 << 31;

		static Int32 Zag( UInt32 ziggedValue )
		{
			var value = (Int32) ziggedValue;
			return -( value & 0x01 ) ^ ( ( value >> 1 ) & ~Int32Msb );
		}

		static Int64 Zag( UInt64 ziggedValue )
		{
			var value = (Int64) ziggedValue;
		...
		public Int64 ReadInt64()
		{
			switch ( wireType )
			{
				...
				case WireType.Fixed64:
					...
					return (Int64) ioBuffer[ioIndex++]
					       | ( (Int64) ioBuffer[ioIndex++] << 8 )
					       | ( (Int64) ioBuffer[ioIndex++] << 16 )
					       | ( (Int64) ioBuffer[ioIndex++] << 24 )
					       | ( (Int64) ioBuffer[ioIndex++] << 32 )
					       | ( (Int64) ioBuffer[ioIndex++] << 40 )
					       | ( (Int64) ioBuffer[ioIndex++] << 48 )
					       | ( (Int64) ioBuffer[ioIndex++] << 56 );
		...
		Int32 TryReadUInt64VariantWithoutMoving( out UInt64 value )
		{
			if ( available < 10 ) Ensure( 10, false );
			if ( available == 0 )
			{
			...
			if ( ( chunk & ~(UInt64) 0x01 ) != 0 ) throw AddErrorData( new OverflowException(), this );
			return 10;
		}

		UInt64 ReadUInt64Variant()
		{
			UInt64 value;
...
#else
		System.Collections.Generic.Dictionary<String, String> stringInterner;
		...
		public String ReadString()
		{
			...
			{
				var bytes = (Int32) ReadUInt32Variant( false );
				if ( bytes == 0 ) return "";
...
#else
				var s = encoding.GetString( ioBuffer, ioIndex, bytes );
#endif
		...
		/// </summary>
		public void ThrowEnumException( Type type, Int32 value )
		{
		...
		Exception CreateWireTypeException() { return CreateException( "Invalid wire-type; this usually means you have over-written a file without truncating or setting the length; see http://stackoverflow.com/q/2152978/23354" ); }

		Exception CreateException( String message ) { return AddErrorData( new ProtoException( message ), this ); }
		...
		/// </summary>
		public static Object ReadObject( Object value, Int32 key, ProtoReader reader )
		{
#if FEAT_IKVM
...
#if !FEAT_IKVM
		internal static Object ReadTypedObject( Object value, Int32 key, ProtoReader reader, Type type )
		{
			if ( reader.model == null ) throw AddErrorData( new InvalidOperationException( "Cannot deserialize sub-objects unless a model is provided" ), reader );
		...
		public static void EndSubItem( SubItemToken token, ProtoReader reader )
		{
			...
			switch ( reader.wireType )
			{
				...
				default:
					...
					if ( reader.blockEnd != reader.position && reader.blockEnd != Int32.MaxValue ) throw reader.CreateException( "Sub-message not read correctly" );
		...
		public static SubItemToken StartSubItem( ProtoReader reader )
		{
			...
			switch ( reader.wireType )
			{
				...
				case WireType.String:
					var len = (Int32) reader.ReadUInt32Variant( false );
					if ( len < 0 ) throw AddErrorData( new InvalidOperationException(), reader );
					var lastEnd = reader.blockEnd;
		...
		/// </summary>
		public Boolean TryReadFieldHeader( Int32 field )
		{
		...
		public void Hint( WireType wireType )
		{
			...
			else if ( ( (Int32) wireType & 7 ) == (Int32) this.wireType )
			{
		...
		public void SkipField()
		{
			switch ( wireType )
			{
				...
				case WireType.Fixed64:
					...
					return;
				case WireType.String:
					var len = (Int32) ReadUInt32Variant( false );
					if ( len <= available )
					{
						// just jump it!
						available -= len;
		...
		public UInt64 ReadUInt64()
		{
			switch ( wireType )
			{
				...
				case WireType.Fixed64:
					...
					return (UInt64) ioBuffer[ioIndex++]
					       | ( (UInt64) ioBuffer[ioIndex++] << 8 )
					       | ( (UInt64) ioBuffer[ioIndex++] << 16 )
					       | ( (UInt64) ioBuffer[ioIndex++] << 24 )
					       | ( (UInt64) ioBuffer[ioIndex++] << 32 )
					       | ( (UInt64) ioBuffer[ioIndex++] << 40 )
					       | ( (UInt64) ioBuffer[ioIndex++] << 48 )
					       | ( (UInt64) ioBuffer[ioIndex++] << 56 );
				default:
					throw CreateWireTypeException();
			}
		}
		...
		//    return buffer;
		//}
		static Int32 ReadByteOrThrow( Stream source )
		{
		...
		/// </summary>
		public static Int32 ReadLengthPrefix( Stream source, Boolean expectHeader, PrefixStyle style, out Int32 fieldNumber )
		{
			Int32 bytesRead;
		...
		/// </summary>
		public static Int32 DirectReadLittleEndianInt32( Stream source )
		{
		...
		/// </summary>
		public static Int32 DirectReadBigEndianInt32( Stream source )
		{
		...
		/// </summary>
		public static Int32 DirectReadVarintInt32( Stream source )
		{
			UInt32 val;
		...
		/// </summary>
		public static void DirectReadBytes( Stream source, Byte[] buffer, Int32 offset, Int32 count )
		{
			Int32 read;
			if ( source == null ) throw new ArgumentNullException( "source" );
		...
		/// </summary>
		public static Byte[] DirectReadBytes( Stream source, Int32 count )
		{
		...
		/// </summary>
		public static String DirectReadString( Stream source, Int32 length )
		{
		...
		/// reader to be created.
		/// </summary>
		public static Int32 ReadLengthPrefix( Stream source, Boolean expectHeader, PrefixStyle style, out Int32 fieldNumber, out Int32 bytesRead )
		{
			fieldNumber = 0;
			switch ( style )
			{
				...
				case PrefixStyle.Base128:
					...
					if ( expectHeader )
					{
						...
						if ( tmpBytesRead > 0 )
						{
							if ( ( val & 7 ) != (UInt32) WireType.String )
								// got a header, but it isn't a string
								throw new InvalidOperationException();
							fieldNumber = (Int32) ( val >> 3 );
							tmpBytesRead = TryReadUInt32Variant( source, out val );
							bytesRead += tmpBytesRead;
							if ( bytesRead == 0 )
					...
					tmpBytesRead = TryReadUInt32Variant( source, out val );
					bytesRead += tmpBytesRead;
					return bytesRead < 0 ? -1 : (Int32) val;
		...
		static Int32 TryReadUInt32Variant( Stream source, out UInt32 value )
		{
			value = 0;
			var b = source.ReadByte();
			if ( b < 0 ) return 0;
			...
			b = source.ReadByte();
			if ( b < 0 ) throw EoF( null );
			value |= (UInt32) b << 28; // can only use 4 bits from this chunk
			if ( ( b & 0xF0 ) == 0 ) return 5;

			throw new OverflowException();
		}

		internal static void Seek( Stream source, Int32 count, Byte[] buffer )
		{
			if ( source.CanSeek )
			{
			...
			{
				Int32 bytesRead;
				while ( count > buffer.Length && ( bytesRead = source.Read( buffer, 0, buffer.Length ) ) > 0 ) count -= bytesRead;
				while ( count > 0 && ( bytesRead = source.Read( buffer, 0, count ) ) > 0 ) count -= bytesRead;
			}
		...
		{
#if !CF && !FX11 && !PORTABLE
			if ( exception != null && source != null && !exception.Data.Contains( "protoSource" ) ) exception.Data.Add( "protoSource", $"tag={source.fieldNumber}; wire-type={source.wireType}; offset={source.position}; depth={source.depth}" );
#endif
			return exception;
		}

		static Exception EoF( ProtoReader source ) { return AddErrorData( new EndOfStreamException(), source ); }
		...
		/// </summary>
		public static Boolean HasSubValue( WireType wireType, ProtoReader source )
		{
			if ( source == null ) throw new ArgumentNullException( "source" );
			// check for virtual end of stream
		...
		internal Int32 GetTypeKey( ref Type type ) { return model.GetKey( ref type ); }

		internal NetObjectCache NetCache => netCache;

		internal Type DeserializeType( String value ) { return TypeModel.DeserializeType( model, value ); }
		...
		/// </summary>
		public Type ReadType() { return TypeModel.DeserializeType( model, ReadString() ); }

		internal void TrapNextObject( Int32 newObjectKey )
		{
			trapCount++;
		...
		internal static ProtoReader Create( Stream source, TypeModel model, SerializationContext context, Int32 len )
		{
			var reader = GetRecycled();
			if ( reader == null ) return new ProtoReader( source, model, context, len );
			Init( reader, source, model, context, len );
			return reader;
		}

#if !PLAT_NO_THREADSTATIC
		[ThreadStatic] static ProtoReader lastReader;

		static ProtoReader GetRecycled()
		{
			var tmp = lastReader;
			lastReader = null;
			return tmp;
