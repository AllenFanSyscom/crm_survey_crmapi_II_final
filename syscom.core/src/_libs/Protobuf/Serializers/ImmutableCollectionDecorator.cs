...
namespace libs.ProtoBuf.Serializers
{
	...
	{
		protected override Boolean RequireAdd => false;
#if !NO_GENERICS
		...
		internal static Boolean IdentifyImmutable( TypeModel model, Type declaredType, out MethodInfo builderFactory, out MethodInfo add, out MethodInfo addRange, out MethodInfo finish )
		{
			...
			switch ( typeArgs.Length )
			{
				case 1:
					...
					break; // fine
				case 2:
					var kvp = model.MapType( typeof( System.Collections.Generic.KeyValuePair<,> ) );
			...
			if ( outerType == null && name == "ImmutableSet" ) outerType = model.GetType( declaredType.Namespace + ".ImmutableHashSet", declaredTypeInfo.Assembly );
			...
			if ( addRange == null )
			{
				var enumerable = model.MapType( typeof( System.Collections.Generic.IEnumerable<> ), false );
				if ( enumerable != null ) addRange = Helpers.GetInstanceMethod( builderFactory.ReturnType, "AddRange", new Type[] { enumerable.MakeGenericType( effectiveType ) } );
			}

			return true;
		}
#endif
		readonly MethodInfo builderFactory, add, addRange, finish;
		internal ImmutableCollectionDecorator
		(
			TypeModel model, Type declaredType, Type concreteType, IProtoSerializer tail, Int32 fieldNumber, Boolean writePacked, WireType packedWireType, Boolean returnList, Boolean overwriteList, Boolean supportNull,
