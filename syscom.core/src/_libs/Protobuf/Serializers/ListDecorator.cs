...
namespace libs.ProtoBuf.Serializers
{
	...
	{
		internal static Boolean CanPack( WireType wireType )
		{
			switch ( wireType )
			{
		...
		readonly Type declaredType, concreteType;

		readonly MethodInfo add;

		readonly Int32 fieldNumber;

		Boolean IsList => ( options & OPTIONS_IsList ) != 0;
		Boolean SuppressIList => ( options & OPTIONS_SuppressIList ) != 0;
		Boolean WritePacked => ( options & OPTIONS_WritePacked ) != 0;
		Boolean SupportNull => ( options & OPTIONS_SupportNull ) != 0;
		Boolean ReturnList => ( options & OPTIONS_ReturnList ) != 0;
		protected readonly WireType packedWireType;


		internal static ListDecorator Create( TypeModel model, Type declaredType, Type concreteType, IProtoSerializer tail, Int32 fieldNumber, Boolean writePacked, WireType packedWireType, Boolean returnList, Boolean overwriteList, Boolean supportNull )
		{
#if !NO_GENERICS
		...
		protected ListDecorator( TypeModel model, Type declaredType, Type concreteType, IProtoSerializer tail, Int32 fieldNumber, Boolean writePacked, WireType packedWireType, Boolean returnList, Boolean overwriteList, Boolean supportNull )
			: base( tail )
		{
		...
		protected virtual Boolean RequireAdd => true;

		public override Type ExpectedType => declaredType;
		public override Boolean RequiresOldValue => AppendToCollection;
		public override Boolean ReturnsValue => ReturnList;

		protected Boolean AppendToCollection => ( options & OPTIONS_OverwriteList ) == 0;
...
#else
		static readonly Type ienumeratorType = typeof( IEnumerator ), ienumerableType = typeof( IEnumerable );
#endif
		protected MethodInfo GetEnumeratorInfo( TypeModel model, out MethodInfo moveNext, out MethodInfo current )
		{
			...
			if ( getEnumerator != null )
			{
				...
				if ( moveNext == null && model.MapType( ienumeratorType ).IsAssignableFrom( iteratorType ) ) moveNext = Helpers.GetInstanceMethod( model.MapType( ienumeratorType ), "MoveNext", null );
				// fully typed
				if ( moveNext != null && moveNext.ReturnType == model.MapType( typeof( Boolean ) )
...
#if !NO_GENERICS
			// try IEnumerable<T>
			var tmp = model.MapType( typeof( System.Collections.Generic.IEnumerable<> ), false );
