...
namespace libs.ProtoBuf.Serializers
{
	...
	{
		Boolean IProtoTypeSerializer.HasCallbacks( TypeModel.CallbackType callbackType ) { return ( (IProtoTypeSerializer) proxy.Serializer ).HasCallbacks( callbackType ); }
		Boolean IProtoTypeSerializer.CanCreateInstance() { return ( (IProtoTypeSerializer) proxy.Serializer ).CanCreateInstance(); }
#if FEAT_COMPILER
...
#endif
#if !FEAT_IKVM
		void IProtoTypeSerializer.Callback( Object value, TypeModel.CallbackType callbackType, SerializationContext context ) { ( (IProtoTypeSerializer) proxy.Serializer ).Callback( value, callbackType, context ); }
		Object IProtoTypeSerializer.CreateInstance( ProtoReader source ) { return ( (IProtoTypeSerializer) proxy.Serializer ).CreateInstance( source ); }
#endif

		readonly Int32 key;
		readonly Type type;
		readonly ISerializerProxy proxy;
		readonly Boolean recursionCheck;

		public SubItemSerializer( Type type, Int32 key, ISerializerProxy proxy, Boolean recursionCheck )
		{
