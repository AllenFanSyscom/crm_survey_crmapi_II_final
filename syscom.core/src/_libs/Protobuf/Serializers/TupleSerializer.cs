	...
	internal sealed class TupleSerializer : IProtoTypeSerializer
	{
		...
		public TupleSerializer( RuntimeTypeModel model, ConstructorInfo ctor, MemberInfo[] members )
		{
			...
			for ( var i = 0; i < members.Length; i++ )
			{
				...
				if ( typeIndex >= 0 ) asReference = model[tmp].AsReferenceDefault;
		...
		public Boolean HasCallbacks( TypeModel.CallbackType callbackType ) { return false; }
		...
		Object GetValue( Object obj, Int32 index )
		{
			PropertyInfo prop;
			FieldInfo field;
		...
		Type GetMemberType( Int32 index )
		{
			var result = Helpers.GetMemberType( members[index] );
