...
namespace libs.ProtoBuf.Serializers
{
	internal sealed class TypeSerializer : IProtoTypeSerializer
	{
		...
		readonly Type forType, constructType;
#if WINRT
        private readonly TypeInfo typeInfo;
#endif
		public Type ExpectedType => forType;
		readonly IProtoSerializer[] serializers;
		readonly Int32[] fieldNumbers;
		readonly Boolean isRootType, useConstructor, isExtensible, hasConstructor;
		readonly CallbackSet callbacks;
		readonly MethodInfo[] baseCtorCallbacks;
		readonly MethodInfo factory;
		public TypeSerializer( TypeModel model, Type forType, Int32[] fieldNumbers, IProtoSerializer[] serializers, MethodInfo[] baseCtorCallbacks, Boolean isRootType, Boolean useConstructor, CallbackSet callbacks, Type constructType, MethodInfo factory )
		{
			...
			for ( var i = 1; i < fieldNumbers.Length; i++ )
			{
				...
				if ( !hasSubTypes && serializers[i].ExpectedType != forType ) hasSubTypes = true;
			}
...
#if !NO_GENERICS
			if ( Helpers.GetUnderlyingType( forType ) != null ) throw new ArgumentException( "Cannot create a TypeSerializer for nullable types", "forType" );
#endif
...
#endif
			if ( constructType != forType && useConstructor && !hasConstructor ) throw new ArgumentException( "The supplied default implementation cannot be created: " + constructType.FullName, "constructType" );
		}
#if WINRT
        private static readonly TypeInfo iextensible = typeof(IExtensible).GetTypeInfo();
#else
		static readonly Type iextensible = typeof( IExtensible );
#endif

		Boolean CanHaveInheritance
		{
			get
			{
#if WINRT
		...
		Boolean IProtoTypeSerializer.CanCreateInstance() { return true; }
#if !FEAT_IKVM
		Object IProtoTypeSerializer.CreateInstance( ProtoReader source ) { return CreateInstance( source, false ); }
		...
		IProtoSerializer GetMoreSpecificSerializer( Object value )
		{
			...
			for ( var i = 0; i < serializers.Length; i++ )
			{
				...
				if ( ser.ExpectedType != forType && Helpers.IsAssignableFrom( ser.ExpectedType, actualType ) ) return ser;
			}
		...
		public Object Read( Object value, ProtoReader source )
		{
			...
			Int32 fieldNumber, lastFieldNumber = 0, lastFieldIndex = 0;
			...
			{
				fieldHandled = false;
				if ( fieldNumber < lastFieldNumber ) lastFieldNumber = lastFieldIndex = 0;
		...
		Object CreateInstance( ProtoReader source, Boolean includeLocalCallback )
		{
			...
			if ( baseCtorCallbacks != null )
				for ( var i = 0; i < baseCtorCallbacks.Length; i++ )
...
#endif
		Boolean IProtoSerializer.RequiresOldValue => true;
		Boolean IProtoSerializer.ReturnsValue => false; // updates field directly
#if FEAT_COMPILER
